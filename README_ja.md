...
最新リリース: [ExtraSync v0.2.0](https://github.com/feilong-pixel/ZeroTraceMobile/releases/tag/v0.2.0)

Android 端末から PC への同期メインフローは、実機の Android 端末と
PC で受け入れ確認済みです。確認済みの範囲には、QR ペアリング、単一
バッチ転送、Auto Sync 転送、フォアグラウンドサービスによるバックグラウンド転送、
Wi-Fi 限定アップロード、やさしい転送、10 件単位の manifest バッチ、PC 側で
読み取れるアップロード済み画像、信頼できる同期件数が含まれます。
詳しくは [Android Sync Acceptance](docs/AndroidSyncAcceptance.md) を参照してください。

1. リリースページから `ExtraSync-v0.2.0-android.apk` をダウンロードします。
...
- 自動連続同期と停止に対応。
- アプリをバックグラウンドにしたり画面をロックしたりしても、実行中の Auto Sync を継続。
- Wi-Fi 接続中のみ写真をアップロード。
- ユーザーが選ぶやさしい転送レベルでアップロード間隔を調整。
